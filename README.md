...
* `http://localhost:8080` (exposed to host) - user `airflow` / passwd `airflow`.
* `sales` database: can be logged on `localhost:5432` (exposed to host) -  user `sales` / passwd `sales`.
* `airflow` metdata db can be connected as well but it's not holding data (port not exposed).
* **In order to run both tasks in the DAG, check the reference below on how to build both the `process-sales-data` image, and the `dbt-process-sales-data`, as they are not part of the `docker-compose` build / deployment.**
